...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        Map<Character, Integer> t = PatternMatching.buildLastTable(pattern);
        int i = 0;
        while (i <= text.length() - pattern.length()) {
            ...
            if (j == -1) {
                ...
                int k = t.getOrDefault(text.charAt(i + j), -1);
                if (k < j) {
                    i += j - k;
                } else {
                    ++i;
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        for (int i = pattern.length() - 1; i >= 0; --i) {
            p.put(pattern.length() - 1 - i, t);
            ph += pattern.charAt(i) * t;
            th += text.charAt(i) * t;
            t *= BASE;
        }
        int i = 0;
        while (i <= text.length() - pattern.length()) {
            ...
            if (++i <= text.length() - pattern.length()) {
                th = (th - text.charAt(i - 1) * p.get(pattern.length() - 1))
